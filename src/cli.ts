...
import { extractPrDependencies, isDepedabotPr, versionIsApproved, checkPendingPrs } from "./dependencies.js"
...
program.name('DittoBot')
    .description('Automatically approves and merges Dependabot PRs across multiple repos once a dependency version has been manually reviewed in any single repo.')
    .version('1.0.0')
...
program.command("approve")
    ...
    .option("-e, --ecosystem <ecosystem>", "The ecosystem to approve for (e.g. npm, pip, etc.)")
    .option("-u, --user <user>", "The GitHub username of the approver", "dittobot-cli")
    .action(async (dependency: string, options: { version?: string, ecosystem?: string, user: string }) => {
        if (!options.version || !options.ecosystem) {
            throw new Error("Error: --version and --ecosystem options are required")
        }
        ...
        for (const [ecosystem, packages] of Object.entries(approvedVersions)) {
            for (const [packageName, versions] of Object.entries(packages)) {
                for (const [version, meta] of Object.entries(versions)) {
                    console.log(`${packageName}==${version} (${ecosystem}) approved by ${meta.approvedBy} on ${meta.approvedAt} in ${meta.sourceRepo}#${meta.sourcePr}`)
                }
            }
        }
    })
    ...
    .action(async () => {
        ...
        for (const repo of config.enrolledRepos) {
            ...
            for (const pr of prs) {
                ...
                const prDeps = await extractPrDependencies(pr, octokit, config.org)
                if (!prDeps) {
                    continue
                }

                const statuses = prDeps.map(dep => ({ dep, approved: versionIsApproved(approvedVersions, dep) }))
                ...
                console.log(`  [${allApproved ? "READY" : "PENDING"}] PR #${pr.number}: ${pr.title}`)
                for (const { dep, approved } of statuses) {
                    console.log(`      ${dep.name}==${dep.version} (${dep.ecosystem}) ${approved ? "APPROVED" : "PENDING"}`)
                }
            }
        }
...
program.command("unenroll")
    ...
    .action(async (repository: string) => {
        ...
        config.enrolledRepos = config.enrolledRepos.filter(repo => repo !== repository)
        await store.updateConfig("enrolledRepos", config.enrolledRepos)
