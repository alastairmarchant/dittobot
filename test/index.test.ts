...
let mockStoreProvider: any

vi.mock("../src/store.js", async () => {
  const store = await vi.importActual<typeof import("../src/store.js")>("../src/store.js")
  return {
    ...store,
    getStoreProvider: vi.fn().mockImplementation((octokit) => {
      ...
      provider.addApprovedVersion("ruff", "0.15.11", "uv", "octocat", "octocat/Hello-World", 28)
...
describe("DittoBot app", () => {
  ...
  ])("Skips non-dependabot pull requests for event %s", async (event, payload) => {
  ...
  test("Skips dependabot pull requests closed by bots", async () => {
    ...
    const payload = JSON.parse(JSON.stringify(prClosedPayload))
  ...
  test("Skips dependabot pull requests approved by bots", async () => {
    ...
    const payload = JSON.parse(JSON.stringify(prReviewSubmittedPayload))
    payload.pull_request.user.login = "dependabot[bot]"
    payload.sender.type = "Bot"
  ...
  test.each([
    ...
    ["pull_request.reopened", prReopenedPayload],
    ["pull_request.synchronize", prSynchronizePayload],
  ])("Merges and creates a comment on %s with approved dependencies", async (event, payload) => {
    const mock = nock("https://api.github.com")
      ...
      .get("/repos/octocat/Team%20Environment/pulls/42/commits", (body: any) => {
        return true
      })
      .reply(200, [
        {
      ...
      // Test that a comment is posted
      .post("/repos/octocat/Team%20Environment/pulls/42/reviews", (body: any) => {
...
_This PR was automatically approved because these versions were manually reviewed in another repository. See the [approval store]() for details._`,
      ...
      .reply(200)
      .put("/repos/octocat/Team%20Environment/pulls/42/merge", (body: any) => {
        return true
      })
      .reply(200)
    ...
    expect(await store.getApprovedVersions()).toMatchObject({
      "uv": {
        "ruff": {
          "0.15.11": {
            approvedBy: "octocat",
            approvedAt: expect.toSatisfy((date) => !isNaN(Date.parse(date))),
      ...
      .reply(200, [
        {
          sha: "abc123",
          commit: {
            message: dependabotCommitMessage.replace(/0\.15\.11/g, "0.15.12").replace(/0\.15\.10/g, "0.15.11"),
          },
        },
      ])
      .get("/repos/octocat/octocat%2FHello-World/pulls?state=open&per_page=100")
      .reply(200, [])
      .get("/repos/octocat/octocat%2FAnother-Repo/pulls?state=open&per_page=100")
      .reply(200, [])
  ...
  test("Merges and stores approval when PR is approved", async () => {
    ...
    await probot.receive({ name: "pull_request_review", payload: prReviewSubmittedPayload })
...
_This PR was automatically approved because these versions were manually reviewed in another repository. See the [approval store]() for details._`,
    ...
    await probot.receive({ name: "check_suite", payload: checkSuiteCompletedPayload })
  ...
  test("Skips check_suite.completed when PR not found", async () => {
    ...
    const payload = JSON.parse(JSON.stringify(checkSuiteCompletedPayload))
